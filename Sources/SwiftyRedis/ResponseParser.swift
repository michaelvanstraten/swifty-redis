...
class RedisResponseParser {
    ...
    func parse_value() throws -> RedisValue {
        if let firstByte = response.popFirst() {
            ...
            case RedisRESP.SimpleString:
                return try .SimpleString(parse_next_line())
    ...
    func parse_int() throws -> RedisValue {
        if let value = try Int64(parse_next_line()) {
            return .Int(value)
        } else {
    ...
    func parse_bulk_string() throws -> RedisValue {
        if case let .Int(length) = try parse_int() {
            if length < 0 {
                return .Nil
            } else {
    ...
    func parse_array() throws -> RedisValue {
        if case let .Int(length) = try parse_int() {
            if length < 0 {
                ...
                return try .Array((1 ... length).map { _ in try parse_value() })
            }
        } else {
            unreachable()
        }
